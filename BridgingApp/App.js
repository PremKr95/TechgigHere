/**
 * Sample React Native App
 * https://github.com/facebook/react-native
 *
 * @format
 * @flow
 */

import React, {Component} from 'react';
import {Platform, StyleSheet, Text, View, TouchableOpacity , NativeModules} from 'react-native';
import BridgeModule from './Bridging.js'
const instructions = Platform.select({
  ios: 'Press Cmd+R to reload,\n' + 'Cmd+D or shake for dev menu',
  android:
    'Double tap R on your keyboard to reload,\n' +
    'Shake or press menu button for dev menu',
});

type Props = {};
export default class App extends Component<Props> {
  callingModule() {
    console.log("hiii");
    BridgeModule.testMethod();
    BridgeModule.testCallBackMethod(X=>{
      console.log(X);
    }) 
  }

  
  render() {
    return (
      <View style={styles.container}>
        {/* <TouchableOpacity 
          onPress={this.callingModule}
          style={{height: 100, width: 100, backgroundColor: 'red'}}></TouchableOpacity>
        <Text style={styles.instructions}>To get started, edit App.js</Text> */}
        <Text onPress={()=>BridgeModule.navigateToExample()} style={styles.instructions}>instructions</Text>
      </View>
    );
  }
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
    backgroundColor: '#F5FCFF',
  },
  welcome: {
    fontSize: 20,
    textAlign: 'center',
    margin: 10,
  },
  instructions: {
    textAlign: 'center',
    color: '#333333',
    marginBottom: 5,
  },
});
